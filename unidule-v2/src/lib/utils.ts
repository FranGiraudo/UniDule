export const t2m = (t: string) => { const [h,m] = t.split(':').map(Number); return h*60+m; };
export const m2t = (m: number) => `${String(Math.floor(m/60)).padStart(2,'0')}:${String(m%60).padStart(2,'0')}`;
