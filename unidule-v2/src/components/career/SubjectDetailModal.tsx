...
export function SubjectDetailModal({ subjectId, onClose }: Props) {
  ...
  const subject = career?.subjects?.find(s => s.id === subjectId || (s as any).code === subjectId);
  ...
  useEffect(() => {
    if (target) {
      const computed = isElective ? (target.status || 'pendiente') : getComputedStatus(target as any, career?.subjects || []);
      setStatus(target.status || computed);
      setGrade(target.grade !== null && target.grade !== undefined ? target.grade.toString() : '');
      setRegDate(target.regDate || '');
      setExpDate(target.expDate || '');
    }
  }, [target, isElective, career?.subjects]);
  ...
  const cfg = CAREER_STATUS_CFG[status as keyof typeof CAREER_STATUS_CFG] || CAREER_STATUS_CFG.pendiente;
  const credits = isElective ? elective.credits : subject?.credits || 0;
  const yearStr = isElective ? `Área: ${elective.category}` : `${subject?.year}° Año · ${subject?.period === 0 ? 'Anual' : subject?.period + '° Semestre'}`;
  ...
  const renderCorrItem = (depId: string) => {
    ...
    const dc = CAREER_STATUS_CFG[dcs as keyof typeof CAREER_STATUS_CFG] || CAREER_STATUS_CFG.pendiente;
    return (
      <div key={dep.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
        <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 6px', borderRadius: '4px', background: dc.bg, color: dc.color, border: `1px solid ${dc.border}` }}>
          {dc.label}
        </span>
  ...
  const unlocks = !isElective ? (career?.subjects?.filter(s => (s.correlatives || []).includes(subject!.id)) || []) : [];

  return (
    <div className="modal-bd" onClick={onClose} style={{ zIndex: 9999 }}>
      <div className="modal-box modal-sm fade-in" onClick={e => e.stopPropagation()} style={{ width: '90%' }}>
        <div className="m-header">
          <div>
            <div className="modal-title" style={{ fontSize: '1.15rem' }}>{target.name}</div>
        ...
        <div className="m-body" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ alignSelf: 'flex-start' }}>
            <span style={{ fontSize: '12px', fontWeight: 700, padding: '4px 10px', borderRadius: '6px', background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.border}` }}>
              {cfg.label}
            </span>
          </div>
          ...
          {(status === 'regular' || status === 'aprobada') && (
            <div className="fade-in" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div>
                ...
                <input type="date" className="f-input" value={regDate} onChange={e => handleRegDateChange(e.target.value)} />
              </div>
              <div>
                <label className="f-label">Vencimiento</label>
                <input type="date" className="f-input" value={expDate} onChange={e => setExpDate(e.target.value)} />
              </div>
            </div>
          )}
          ...
          {!isElective && (
            <>
              <div style={{ marginTop: '4px', paddingTop: '16px', borderTop: '1px solid var(--border)' }}>
                <div className="f-label" style={{ marginBottom: '8px' }}>Para cursar (necesitás regular)</div>
                {toCurse.length > 0 ? toCurse.map(depId => renderCorrItem(depId)) : <div style={{ fontSize: '12px', color: 'var(--text2)' }}>Sin correlativas</div>}
              </div>

              <div>
                <div className="f-label" style={{ marginBottom: '8px' }}>Desbloquea (al regularizar)</div>
                {unlocks.length > 0 ? unlocks.map(dep => renderCorrItem(dep.id)) : <div style={{ fontSize: '12px', color: 'var(--text2)' }}>—</div>}
              </div>
            </>
          )}
        </div>
