...
export function GradesModal({ subject, onClose }: Props) {
  ...
  const addGrade = () =>
    setGrades([...grades, { id: crypto.randomUUID(), type: 'Parcial 1', score: '', date: '' }]);
        ...
        <div className="m-body" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div>
            ...
            >
              ...
              <button type="button" className="btn btn-ghost btn-sm" onClick={addGrade}>
                ...
                return (
                  <div key={g.id} className="grade-row">
                    <select
                      ...
                      style={{ fontSize: '12px' }}
                      value={g.type}
                      onChange={(e) => updGrade(i, 'type', e.target.value)}
                    >
                      {GRADE_TYPES.map((t) => (
                    ...
                    <input
                      ...
                      style={{ fontSize: '12px' }}
                      value={g.date || ''}
                      onChange={(e) => updGrade(i, 'date', e.target.value)}
                    />
                    <div className="slot-rm" onClick={() => rmGrade(i)}>
                      ✕
                    </div>
                  </div>
                );
