...
export function GradePromptModal({ task, onClose }: Props) {
  ...
  const [score, setScore] = useState(grade && grade.score !== '' && grade.score !== null ? String(grade.score) : '');
  ...
  const handleSave = async () => {
    if (!subject || !grade) { onClose(); return; }
    const raw = score.trim();
    if (raw !== '') {
      const num = parseFloat(raw);
      if (isNaN(num) || num < 0 || num > 10) { alert('Nota inválida. Ingresá un número entre 0 y 10.'); return; }
    }
    setSaving(true);
    try {
      const newScore: number | '' = raw === '' ? '' : parseFloat(raw);
      const newGrades = subject.grades!.map(g => (g.id === grade.id ? { ...g, score: newScore } : g));
  ...
  return (
    <div className="modal-bd" onClick={onClose} style={{ zIndex: 9999 }}>
      <div className="modal-box fade-in" onClick={e => e.stopPropagation()} style={{ width: '90%', maxWidth: '360px' }}>
        <div className="m-header">
          <div className="modal-title">Anotar nota</div>
          <button className="btn-icon" onClick={onClose}><X size={20} /></button>
        </div>
        <div className="m-body">
          <div style={{ fontSize: '13px', color: 'var(--text2)', marginBottom: '10px' }}>{task.title}</div>
          <input
            autoFocus
