...
export function MapTab({ onSelectSubject }: { onSelectSubject: (id: string) => void }) {
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      ...
      setTransform((t) => ({
        ...t,
        s: Math.max(0.22, Math.min(2.5, t.s + (e.deltaY > 0 ? -0.03 : 0.03))),
      }));
    };
  ...
  const { posMap, cols, SVG_W, SVG_H } = useMemo(() => {
    ...
    const markOccupied = (c: number, r: number) => {
      if (!grid[c]) grid[c] = {};
      grid[c][r] = true;
    };
    ...
    const maxR = Math.max(
      ...Object.values(cols).map((c) =>
        c.length > 0 ? Math.max(...c.map((s) => s.layout_row ?? 0)) + 1 : 0,
      ),
      0,
    );
    ...
    Object.entries(cols).forEach(([ci, colSubs]) => {
      colSubs.forEach((s) => {
        ...
        pm[s.id] = {
          x: CM.M + parseInt(ci) * COL_W,
          y: CM.HEADER + (s.layout_row ?? 0) * ROW_H,
          w,
        };
      });
    });
  ...
  const V = {
    bg: 'var(--bg)',
    nodeFill: 'var(--card)',
    nodeFillBloq: 'var(--bg2)',
    nodeFillDisp: 'var(--card2)',
  ...
  const getStatusStyle = (s: Subject) => {
    switch (getComputedStatus(s, subjects)) {
      ...
      default:
        return { bg: V.nodeFill, stroke: V.nodeStroke, text: V.textDark, label: '' };
    }
  };
    ...
    <div className="fade-in" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        <button
          ...
          onClick={() => setTransform((t) => ({ ...t, s: Math.min(2.5, t.s + 0.15) }))}
        >
          ＋
        </button>
        <button
          className="btn btn-ghost btn-sm"
          onClick={() => setTransform((t) => ({ ...t, s: Math.max(0.22, t.s - 0.15) }))}
        >
          －
        </button>
        <button
          className="btn btn-ghost btn-sm"
          onClick={() => setTransform({ x: 0, y: 0, s: 0.72 })}
        >
          Centrar Mapa
        </button>
        <div
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <div style={{ width: '10px', height: '10px', background: V.nodeFillDisp }}></div>
            Disponible
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <div style={{ width: '10px', height: '10px', background: V.nodeFillC }}></div>Cursando
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <div style={{ width: '10px', height: '10px', background: V.nodeFillR }}></div>Regular
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <div style={{ width: '10px', height: '10px', background: V.nodeFillA }}></div>Aprobada
          </div>
        </div>
      </div>
          ...
          <g>
            ...
            {subjects.map((tgt) =>
              (tgt.correlatives || []).map((srcId) => {
                ...
                if (hoverId) {
                  if (tgt.id === hoverId) {
                    stroke = V.edgeIn;
                    marker = 'url(#cm-arr-in)';
                    opacity = 1;
                    sw = 2;
                  } else if (srcId === hoverId) {
                    stroke = V.edgeOut;
                    marker = 'url(#cm-arr-out)';
                    opacity = 1;
                    sw = 2;
                  } else {
                    opacity = 0.2;
            ...
            {subjects.map((s) => {
              ...
              if (
                hoverId &&
                hoverId !== s.id &&
                !neededByHover.has(s.id) &&
                !unlockedByHover.has(s.id)
              ) {
                opacity = 0.25;
              }
