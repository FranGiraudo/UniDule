...
export function MapTab({ onSelectSubject }: { onSelectSubject: (id: string) => void }) {
  ...
  const { posMap, cols, SVG_W, SVG_H } = useMemo(() => {
    ...
    const maxR = Math.max(...Object.values(cols).map(c => c.length > 0 ? Math.max(...c.map(s => s.layout_row ?? 0)) + 1 : 0), 0);
    ...
    Object.entries(cols).forEach(([ci, colSubs]) => {
      colSubs.forEach(s => {
        ...
        pm[s.id] = { x: CM.M + parseInt(ci) * COL_W, y: CM.HEADER + (s.layout_row ?? 0) * ROW_H, w };
      });
    });
