...
export function StatsTab() {
  ...
  const stats = useMemo(() => {
    ...
    const disponible = subjects.filter(
      (s) => getComputedStatus(s, subjects) === 'disponible',
    ).length;
    ...
    const avgPass = passGrades.length
      ? (passGrades.reduce((a, b) => a + b, 0) / passGrades.length).toFixed(2)
      : '—';

    const totalCred = subjects.reduce((a, s) => a + (s.credits || 0), 0);
    const approvedCred = subjects
      ...
      <div className="premium-card premium-ring-col">
        ...
        >
          ...
          <circle
            ...
            r={R}
            fill="none"
            stroke="url(#gradIngProy)"
            strokeWidth="14"
            strokeDasharray={C}
            strokeDashoffset={C * (1 - stats.pctProy / 100)}
            strokeLinecap="round"
          ...
          <circle
            ...
            strokeWidth="14"
            strokeDasharray={C}
            strokeDashoffset={C * (1 - stats.pctActual / 100)}
            strokeLinecap="round"
            transform="rotate(-90 80 80)"
          ...
          <text x="80" y="108" textAnchor="middle" fill="#a78bfa" fontSize="9" fontWeight="600">
            PROY: {stats.pctProy}%
          </text>
        </svg>
        ...
        <div className="ring-stats-box">
          <div className="ring-stat-item">
            <span className="ring-stat-val" style={{ color: '#4ade80' }}>
              {stats.aprobadas}
            </span>
          ...
          <div className="ring-stat-item">
            <span className="ring-stat-val" style={{ color: '#a78bfa' }}>
              {stats.regulares}
            </span>
      ...
      <div className="premium-card premium-ring-col">
        ...
        >
          ...
          <circle
            ...
            r={R}
            fill="none"
            stroke="url(#gradAnaProy)"
            strokeWidth="14"
            strokeDasharray={C}
            strokeDashoffset={C * (1 - stats.pctAnProy / 100)}
            strokeLinecap="round"
            transform="rotate(-90 80 80)"
            style={{ transition: 'stroke-dashoffset 1s cubic-bezier(0.4,0,0.2,1)' }}
          />
          <circle
            ...
            strokeWidth="14"
            strokeDasharray={C}
            strokeDashoffset={C * (1 - stats.pctAnActual / 100)}
            strokeLinecap="round"
            transform="rotate(-90 80 80)"
            style={{ transition: 'stroke-dashoffset 1s cubic-bezier(0.4,0,0.2,1) 0.2s' }}
          />
          ...
          <text x="80" y="108" textAnchor="middle" fill="#fbbf24" fontSize="9" fontWeight="600">
            PROY: {stats.pctAnProy}%
          </text>
        </svg>

        <div className="ring-stats-box">
          <div className="ring-stat-item">
            <span className="ring-stat-val" style={{ color: '#60a5fa' }}>
              {stats.anAprob}
            </span>
            <span className="ring-stat-lbl">Aprobadas</span>
          </div>
          <div style={{ width: '1px', background: 'rgba(255,255,255,0.1)' }}></div>
          <div className="ring-stat-item">
            <span className="ring-stat-val" style={{ color: 'var(--text2)' }}>
              {stats.anTotal}
            </span>
      ...
      <div className="mini-stats-grid">
        {stats.miniGrid.map(([lbl, val, col], i) => (
          <div
            ...
            style={{ animation: `popIn 0.4s ease-out ${0.1 * i}s both` }}
          >
            <div
              style={{
                fontSize: '2rem',
