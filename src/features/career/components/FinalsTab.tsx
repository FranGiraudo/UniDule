...
export function FinalsTab({ onSelectSubject }: { onSelectSubject: (id: string) => void }) {
      ...
      <div className="fade-in">
        ...
        >
          ...
          <div style={{ fontSize: '0.9375rem', fontWeight: 700, color: 'var(--text)' }}>
            No hay materias regulares registradas
          </div>
          <div style={{ fontSize: '0.8125rem', marginTop: '0.25rem' }}>
            Cuando tengas materias regularizadas aparecerán aquí con sus fechas de vencimiento.
          </div>
        </div>
      </div>
    );
  ...
  const filteredSubs = regularSubs.filter((s) => {
    if (search) {
        ...
        >
          {filteredSubs.map((s) => {
            ...
            let badgeBg = 'rgba(74,222,128,.15)',
              badgeColor = '#4ade80',
              badgeText = 'Regularidad activa';

            if (daysLeft !== null) {
              if (daysLeft < 0) {
                badgeBg = 'rgba(248,113,113,.2)';
                badgeColor = '#f87171';
                badgeText = '¡Vencida!';
              } else if (daysLeft <= 90) {
                badgeBg = 'rgba(248,113,113,.15)';
                badgeColor = '#f87171';
            ...
            const regFormatted = regDate
              ? new Date(regDate.replace(/-/g, '/')).toLocaleDateString('es-AR')
              : '—';
            const expFormatted = expDate
              ? new Date(expDate.replace(/-/g, '/')).toLocaleDateString('es-AR')
              : '—';
            ...
            return (
              ...
              >
                <div>
                  ...
                  <div
                    style={{ fontSize: '0.6875rem', color: 'var(--text2)', marginBottom: '0.5rem' }}
                  >
                    Código: {s.id}
                  </div>
                  ...
                  {!canRendir && (
                    <div style={{ fontSize: '0.6875rem', color: '#f87171', marginTop: '0.25rem' }}>
                      Falta aprobar: {missingToPass.map((m) => (m ? m.name : '?')).join(', ')}
                    </div>
                  )}
                </div>
