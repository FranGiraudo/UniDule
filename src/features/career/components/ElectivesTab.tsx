      ...
      >
        <div>
          <div style={{ fontSize: '1rem', fontWeight: 800, color: 'var(--text)' }}>
            Optativas de {career?.name || 'Ingeniería'}
          </div>
          <div style={{ fontSize: '0.75rem', color: 'var(--text2)', marginTop: '0.15rem' }}>
            Selección y acreditación de materias optativas de especialización
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ fontSize: '0.8125rem', fontWeight: 700, color: 'var(--text)' }}>
            {approved} Aprobadas · {cursando} Cursando
          </div>
        </div>
      </div>
        ...
        {electives.map((s) => {
          ...
          return (
            ...
            >
              ...
              >
                <button className="btn btn-ghost btn-sm" onClick={() => onSelectSubject(s.id)}>
                  Gestionar Optativa
                </button>
              </div>
            </div>
