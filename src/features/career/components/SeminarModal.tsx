      ...
      <div className="modal-box modal-sm fade-in" onClick={e => e.stopPropagation()} style={{ width: '90%' }}>
        ...
        <div className="m-body" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div>
            ...
            <input type="text" className="f-input" value={name} onChange={e => setName(e.target.value)} placeholder="Ej: Curso de React Avanzado" />
          </div>
          
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div>
              <label className="f-label">Código (Opcional)</label>
              <input type="text" className="f-input" value={code} onChange={e => setCode(e.target.value)} placeholder="Ej: CS-101" />
            </div>
            <div>
              <label className="f-label">Horas / Créditos</label>
              <input type="number" className="f-input" value={hours} onChange={e => setHours(e.target.value)} placeholder="Ej: 40" min="0" />
            </div>
          </div>

          <div>
            <label className="f-label">Categoría</label>
            <input type="text" className="f-input" value={category} onChange={e => setCategory(e.target.value)} placeholder="Ej: Seminario, Certificación..." />
          </div>
          ...
          <div>
            ...
            <textarea className="f-input" value={notes} onChange={e => setNotes(e.target.value)} rows={2} placeholder="Resolución, detalles..." />
          </div>
        </div>

        <div className="m-footer">
          <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }} onClick={handleSave} disabled={loading}>
            {loading ? 'Guardando...' : <><Save size={16} /> Guardar Seminario</>}
          </button>
        </div>
      </div>
    </div>
