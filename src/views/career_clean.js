...
// ÔöÇÔöÇÔöÇ Estado de filtros para Carrera y Finales ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  if (view && !document.getElementById('career-grid-container')) {
    view.innerHTML = `
      <div style="padding:0 0 1rem;">
        ...
        <div class="view-sub">Ingenier├¡a en Inform├ítica ÔÇö UTN</div>
      </div>
  ...
  el.innerHTML = `
    <div style="margin-bottom:1rem;background:var(--card);border:1px solid var(--border);border-radius:0.75rem;padding:1rem;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:0.75rem;">
      <div>
        ...
        <div style="font-size:0.75rem;color:var(--text2);margin-top:0.15rem;">Control de seminarios exigidos para la titulaci├│n de grado</div>
      </div>
        ...
        return `
          <div style="background:var(--card);border:1px solid var(--border);border-radius:0.75rem;padding:1rem;display:flex;flex-direction:column;justify-content:space-between;gap:0.75rem;">
            <div>
              ...
              <div style="font-size:0.6875rem;color:var(--text2);margin-bottom:0.5rem;">C├│digo: ${s.code || 'ÔÇö'} ┬À ${s.category}</div>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;gap:0.5rem;padding-top:0.5rem;border-top:1px solid var(--border);">
              <span style="font-size:0.6875rem;color:var(--text2);">${s.date ? 'Acreditado: ' + s.date : 'Fecha: ÔÇö'}</span>
...
export function renderCareerElectives() {
  ...
  el.innerHTML = `
    <div style="margin-bottom:1rem;background:var(--card);border:1px solid var(--border);border-radius:0.75rem;padding:1rem;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:0.75rem;">
      <div>
        <div style="font-size:1rem;font-weight:800;color:var(--text);">Optativas de Ingenier├¡a en Inform├ítica</div>
        <div style="font-size:0.75rem;color:var(--text2);margin-top:0.15rem;">Selecci├│n y acreditaci├│n de materias optativas de especializaci├│n</div>
      </div>
      <div style="display:flex;align-items:center;gap:0.75rem;">
        <div style="font-size:0.8125rem;font-weight:700;color:var(--text);">${approved} Aprobadas ┬À ${cursando} Cursando</div>
      </div>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(300px, 1fr));gap:0.75rem;">
      ${elecs.map(s => {
        ...
        return `
          <div style="background:var(--card);border:1px solid var(--border);border-radius:0.75rem;padding:1rem;display:flex;flex-direction:column;justify-content:space-between;gap:0.75rem;">
            <div>
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:0.5rem;margin-bottom:0.375rem;">
                <div style="font-size:0.9375rem;font-weight:700;color:var(--text);">${s.name}</div>
                <span style="font-size:0.625rem;font-weight:700;padding:0.125rem 0.375rem;border-radius:0.25rem;background:color-mix(in srgb, var(--primary) 15%, transparent);color:var(--primary);white-space:nowrap;">${s.credits} cr├®ditos</span>
              </div>
              <div style="font-size:0.6875rem;color:var(--text2);margin-bottom:0.5rem;">C├│digo: ${s.code || 'ÔÇö'} ┬À ├ürea: ${s.category}</div>
...
// ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
//  TAB 1: GRILLA (Resumen por A├▒o / Semestre con Filtros)
// ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
  ...
  const filterBarHtml = `
    <div style="margin-bottom:1rem;display:flex;flex-wrap:wrap;gap:0.5rem;align-items:center;justify-content:space-between;background:var(--card);padding:0.75rem;border-radius:0.75rem;border:1px solid var(--border);">
      <div style="display:flex;align-items:center;gap:0.5rem;flex:1;min-width:200px;">
        <input type="text" class="f-input" placeholder="Buscar materia o c├│digo..."
      ...
      <div style="display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;">
        <select class="f-input" onchange="setCareerGridFilter(this.value)" style="font-size:0.75rem;padding:0.375rem 0.625rem;">
          ...
          <option value="aprobada"  ${careerGridFilter==='aprobada'?'selected':''}>­ƒƒó Aprobadas</option>
          <option value="regular"   ${careerGridFilter==='regular'?'selected':''}>­ƒƒú Regulares</option>
          <option value="cursando"  ${careerGridFilter==='cursando'?'selected':''}>­ƒöÁ Cursando</option>
          <option value="disponible"${careerGridFilter==='disponible'?'selected':''}>­ƒƒí Disponibles</option>
          <option value="bloqueada" ${careerGridFilter==='bloqueada'?'selected':''}>ÔÜ¬ Pendientes/Bloqueadas</option>
        </select>
      </div>
    </div>`;
    ...
    <div class="career-year-block">
      <div class="career-year-header" onclick="this.closest('.career-year-block').classList.toggle('collapsed')">
        <div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0;">
          <span class="career-year-num">${year}┬░ A├▒o</span>
        ...
        <span class="career-chevron">Ôû¥</span>
      </div>
  ...
  if (!regularSubs.length) {
    el.innerHTML = `
      <div style="text-align:center;padding:2.5rem 1rem;color:var(--text2);background:var(--card);border-radius:0.75rem;border:1px solid var(--border);">
        ...
        <div style="font-size:0.8125rem;margin-top:0.25rem;">Cuando tengas materias regularizadas aparecer├ín aqu├¡ con sus fechas de vencimiento.</div>
      </div>`;
    return;
  }
  ...
  const filterBarHtml = `
    <div style="margin-bottom:1rem;display:flex;flex-wrap:wrap;gap:0.5rem;align-items:center;justify-content:space-between;background:var(--card);padding:0.75rem;border-radius:0.75rem;border:1px solid var(--border);">
      <div style="display:flex;align-items:center;gap:0.5rem;flex:1;min-width:180px;">
        <input type="text" class="f-input" placeholder="Buscar final o c├│digo..."
      ...
      <div style="display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;">
        ...
        <select class="f-input" onchange="setFinalsSort(this.value)" style="font-size:0.75rem;padding:0.375rem 0.625rem;">
          <option value="exp-asc" ${finalsSort==='exp-asc'?'selected':''}>Vencimiento m├ís pr├│ximo</option>
          <option value="exp-desc" ${finalsSort==='exp-desc'?'selected':''}>Vencimiento m├ís lejano</option>
          <option value="name-asc" ${finalsSort==='name-asc'?'selected':''}>Nombre (A - Z)</option>
          <option value="year-asc" ${finalsSort==='year-asc'?'selected':''}>A├▒o (1┬░ a 5┬░)</option>
        </select>
      </div>
    </div>`;
  ...
  if (!filteredSubs.length) {
    el.innerHTML = filterBarHtml + `
      <div class="empty-st" style="background:var(--card);border-radius:0.75rem;border:1px solid var(--border);">
        ...
        <div>No se encontraron finales que coincidan con la b├║squeda o filtro seleccionado.</div>
      </div>`;
    return;
  }
  ...
  const cardsHtml = `
    <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(300px, 1fr));gap:0.75rem;">
      ${filteredSubs.map(s => {
        ...
        if (daysLeft !== null) {
          if (daysLeft < 0) { badgeBg = 'rgba(248,113,113,.2)'; badgeColor = '#f87171'; badgeText = '┬íVencida!'; }
          else if (daysLeft <= 90) { badgeBg = 'rgba(248,113,113,.15)'; badgeColor = '#f87171'; badgeText = `Vence en ${daysLeft} d├¡as`; }
          else if (daysLeft <= 180) { badgeBg = 'rgba(251,191,36,.15)'; badgeColor = '#fbbf24'; badgeText = `Vence en ${daysLeft} d├¡as`; }
          else { badgeBg = 'rgba(74,222,128,.15)'; badgeColor = '#4ade80'; badgeText = `Vence en ${daysLeft} d├¡as`; }
        }

        const regFormatted = s.regDate ? new Date(s.regDate.replace(/-/g,'/')).toLocaleDateString('es-AR') : 'ÔÇö';
        const expFormatted = s.expDate ? new Date(s.expDate.replace(/-/g,'/')).toLocaleDateString('es-AR') : 'ÔÇö';
          ...
          <div style="background:var(--card);border:1px solid var(--border);border-radius:0.75rem;padding:1rem;display:flex;flex-direction:column;justify-content:space-between;gap:0.75rem;cursor:pointer;" onclick="openCareerSubDetail('${s.id}')">
            <div>
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:0.5rem;margin-bottom:0.375rem;">
                ...
                <span style="font-size:0.625rem;font-weight:700;padding:0.125rem 0.375rem;border-radius:0.25rem;background:color-mix(in srgb, var(--primary) 15%, transparent);color:var(--primary);white-space:nowrap;">${s.year}┬░ A├▒o</span>
              </div>
              <div style="font-size:0.6875rem;color:var(--text2);margin-bottom:0.5rem;">C├│digo: ${s.code || 'ÔÇö'} ┬À ${s.credits} cr├®ditos</div>
...
export function renderCareerStats() {
  ...
  // 1. T├¡tulo de Grado (Ingeniero en Inform├ítica)
  ...
  // 2. T├¡tulo Intermedio (Analista de Sistemas Inform├íticos - A├▒os 1 a 3)
  ...
  const avg = allGrades.length ? (allGrades.reduce((a, b) => a + b, 0) / allGrades.length).toFixed(2) : 'ÔÇö';
  const avgPass = passGrades.length ? (passGrades.reduce((a, b) => a + b, 0) / passGrades.length).toFixed(2) : 'ÔÇö';
  ...
  el.innerHTML = `
    <div class="cs-layout">
      <!-- Card Ing. Inform├ítica -->
      <div class="cs-card cs-ring-card">
        <div style="font-size:0.75rem;font-weight:700;color:var(--primary);text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.25rem;">T├¡tulo de Grado</div>
        <div style="font-size:1rem;font-weight:900;color:var(--text);margin-bottom:0.75rem;text-align:center;">Ingenier├¡a en Inform├ítica</div>
        ...
        <div style="font-size:0.75rem;color:var(--text2);text-align:center;margin-top:0.5rem;">
          <b>${aprobadas}</b> aprobadas ┬À <b>${regulares}</b> regulares
        </div>
      </div>

      <!-- Card T├¡tulo Intermedio -->
      <div class="cs-card cs-ring-card" style="border-color:rgba(96,165,250,.3);">
        <div style="font-size:0.75rem;font-weight:700;color:#60a5fa;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.25rem;">T├¡tulo Intermedio</div>
        <div style="font-size:1rem;font-weight:900;color:var(--text);margin-bottom:0.75rem;text-align:center;">Analista de Sistemas Inform├íticos</div>
        ...
        <svg width="150" height="150" viewBox="0 0 160 160">
          ...
          <text x="80" y="88" text-anchor="middle" fill="#60a5fa" font-size="9" font-weight="700">Actual (1┬║-3┬║ a├▒o)</text>
      ...
      <div class="cs-mini-grid" style="grid-column: 1 / -1;">
        ${(()=>{
          ...
          return [
            ...
            ['Cr├®ditos Aprob.',  approvedCred,c[6]],
            ['Cr├®ditos Total',   totalCred,   c[7]],
          ].map(([lbl,val,col])=>`
      ...
      <div class="cs-card cs-histo-card">
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:var(--text2);margin-bottom:14px;">Distribuci├│n de Notas (Ex├ímenes Finales)</div>
...
export function openCareerSubDetail(id) {
  ...
  const unlocksHtml = all.filter(x=>(x.correlatives&&x.correlatives.toCurse||[]).includes(id))
    ...
    || '<div style="font-size:11px;color:var(--text2);">ÔÇö</div>';
  ...
  document.getElementById('career-detail-meta').textContent  = isElective 
      ? `Categor├¡a: ${s.category} ┬À ${s.credits} cr├®ditos`
      : `${s.year}┬░ A├▒o ┬À ${s.semester}┬░ Semestre ┬À ${s.credits} cr├®ditos`;
  document.getElementById('career-detail-body').innerHTML = `
    <div style="display:flex;flex-direction:column;gap:16px;">
      ...
      <div id="cd-grade-row" style="${s.status==='aprobada'?'':'display:none;'}">
        <label class="f-label">Nota final (0 ÔÇô 10)</label>
      ...
      <div>
        <div class="f-label" style="margin-bottom:6px;">Para cursar (necesit├ís regular)</div>
        ${needsHtml}
      </div>
      <div>
...
// ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
//  VIEW: CONFIGURACI├ôN & PERFIL & TEMAS & BACKUP (CSV / JSON)
// ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
