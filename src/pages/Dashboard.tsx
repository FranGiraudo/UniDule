...
export function Dashboard() {
  ...
  const warnSubs: Subject[] = subjects.filter(s => s.activeId && (s.absences || 0) >= (s.maxAbsences || 6) * 0.75);
            ...
            <div className="alert-tooltip">
              {warn ? (
                <>
                  ...
                  {warnSubs.map(s => <div key={s.id} style={{ fontSize: '10px', marginTop: '2px' }}>• <strong>{s.name}</strong> ({s.absences}/{s.maxAbsences} faltas)</div>)}
                </>
              ) : (
